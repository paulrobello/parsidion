...
Reads a Gemini hook payload from stdin, builds non-AI session context using the
existing Parsidion SessionStart implementation, and emits valid JSON for Gemini.
All errors are reported to stderr while stdout remains valid JSON so the hook
never blocks Gemini startup.
"""
