...
Reads a Gemini SessionEnd payload from stdin, validates Gemini transcript paths,
parses model text from Gemini JSONL records, updates the vault daily note, and
queues pending summarization when useful categories are detected. The hook always
emits valid JSON on stdout and falls back to ``{}`` on errors.
"""
