...
uv run install.py connect antigravity    # wires ~/.gemini/GEMINI.md + Antigravity hooks
...
| `codex_cli` | `command`, `timeout`, `sandbox`, `ephemeral`, `skip_git_repo_check`, `minimal_context`, `suppress_notify`, `allow_danger_full_access` | ai_backend.py |
...
1. **Hook scripts** — Python scripts fired by Claude Code's lifecycle events, communicating via JSON stdin/stdout:
   ...
   - `user_prompt_submit_hook.py`: Registered under `UserPromptSubmit` for Claude Code and Codex, and called by the omp extension's `before_agent_start` handler. Queries parsight before each turn, applies a distinct-term relevance gate, and injects bounded note summaries inside SEC-108 untrusted-content framing. It fails open with `{}` on unavailable or irrelevant recall.
   ...
   - **Runtime adapter hooks** (registered by `install.py connect <agent>`): `codex_session_start_hook.py` / `codex_stop_hook.py` / `codex_subagent_stop_hook.py` wrap the same context-builder and queue path for Codex CLI (registered in `~/.codex/hooks.json`, native Codex hook lifecycle); `antigravity_session_start_hook.py` / `antigravity_session_end_hook.py` do the same for Antigravity CLI (`agy`, registered in `~/.gemini/config/hooks.json`). pi uses a TypeScript extension (`extensions/pi/parsidion/`) that shells out to the same Python scripts. Since ARC-002 every runtime — Claude's `session_stop_hook.py` included — runs the same session-end pipeline in `agent_adapter.run_session_end`, so all adapters classify, queue to the same `pending_summaries.jsonl`, update daily notes, commit the vault, and trigger the auto-summarizer identically (each stage config-gated by the same `session_stop_hook.*` / `git.auto_commit` keys).
