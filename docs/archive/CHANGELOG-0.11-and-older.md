# Changelog Archive — 0.1.0 through 0.11.x
...
> **Note:** Versions 0.1.0 through 0.5.x were released under the name **`parsidion-cc`**.
> The project was renamed to **`parsidion`** in v0.7.0 (see [0.6.0] for the rebrand
> details). The GitHub repository was also renamed from `paulrobello/parsidion-cc`;
> legacy links redirect automatically.
...
- **`vault_doctor.py` decomposed** — The 3,128-line God module is now a ~180-line thin re-export shim over a `doctor/` package (ARC-008 / QA-003): focused submodules (`check`, `headings`, `worker`, `tags`, `graph`, `prefixes`, `daily`, `permissions`, `frontmatter`, `subfolder`, `links`, `_state`, `protocol`, `orchestrator`, `cli`) behind a `FixMode` dataclass + `run_fix_modes` registry loop that replaces the 55-branch `--fix-all` dispatch ladder — adding a fix-mode is one tuple append, not a branch in two places. Every public and private symbol the original exposed stays importable from `vault_doctor`, so `import vault_doctor` consumers and test `monkeypatch` calls are unchanged, and `uv run --no-project vault_doctor.py` still works.
