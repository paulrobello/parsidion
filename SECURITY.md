...
`~/.gemini/`). This makes the hook execution surface security-sensitive. AI features call a
configured prompt backend (`ai.backend`: `claude-cli`, `codex-cli`, or `grok-cli`) as a subprocess
with an allowlisted environment — configured backend binaries are ownership- and
permission-checked before execution. The pi and omp runtimes ship a shared
TypeScript extension (no hook registration; omp installs the same extension into
`~/.omp/agent/extensions`), and opt-in **external adapter loading**
...
**Supported versions:** the latest minor release only, on Python 3.13+ (the floor declared by `requires-python`).
...
| Shared library (stdlib-only, see constraint below) | `skills/parsidion/scripts/vault_common.py` re-export facade plus the `core/` implementations (`core/vault_config.py`, `vault_schema.py`, `vault_path.py`, `vault_fs.py`, `vault_index.py`, `vault_hooks.py`, `vault_adaptive.py`, `vault_links.py`, `vault_constants.py`, `vault_metrics.py`, `vault_health.py`, `subproc_util.py`, `core/ai_backend.py`, `core/parsight_backend.py`) and the flat re-export shims (`vault_config.py`, `vault_path.py`, `vault_fs.py`, `vault_index.py`, `vault_hooks.py`, `vault_adaptive.py`, `vault_links.py`, `vault_metrics.py`, `vault_constants.py`, `vault_health.py`, `vault_tui.py`, `subproc_util.py`, `ai_backend.py`, `parsight_backend.py`); plus `agent_adapter.py`, `prompt_templates.py`, `note_schema.py` | Vault path resolution, subprocess environment, SQLite access, file locking, prompt-AI backend selection, parsight bridge, adapter registry, prompt rendering. Imports cleanly under the stdlib-only enforcement test |
...
| Session summarizer | `skills/parsidion/scripts/summarize_sessions.py` | Processes transcript content via the configured prompt AI backend (a `claude`/`codex`/`grok` CLI subprocess with an allowlisted env); writes vault notes from AI-generated content |
...
12 third-party modules poisoned in `sys.modules` (`rich`, `fastembed`, `sqlite_vec`/`sqlitevec`,
`anyio`, `yaml`/`pyyaml`, `numpy`, `PIL`/`pillow`, `requests`, and `aiohttp`); a violation —
even a transitive one — fails CI.

**In scope (enforced):**

- The `core/` library package: `vault_config`, `vault_schema`, `vault_path`, `vault_fs`,
  `vault_index`, `vault_hooks`, `vault_adaptive`, `vault_links`, `vault_constants`,
  `vault_metrics`, `vault_health`, `subproc_util`, `ai_backend`, `parsight_backend`
- The flat re-export shims and facade: `vault_common`, `vault_config`, `vault_path`,
  `vault_fs`, `vault_index`, `vault_hooks`, `vault_adaptive`, `vault_links`, `vault_metrics`,
  `vault_constants`, `vault_health`, `vault_tui`, `subproc_util`, `ai_backend`, `parsight_backend`
- Supporting libraries imported by hooks: `agent_adapter`, `vault_health`, `prompt_templates`,
  `note_schema`
