...
- [ ] **ENH-009 — Serve vault resolution through parsidion-mcp** — The vault-precedence contract is implemented twice (Python `core/vault_path.py:resolve_vault` and TS `visualizer/lib/vaultResolver.ts:resolveVault`), and the two already diverge (the TS resolver supports fewer channels — see ARC-007/SEC-P001). Serve resolution through the parsidion-mcp server (which already runs Python and can read `vaults.yaml` authoritatively) and have the visualizer call it, eliminating the cross-language duplication and the drift class entirely. (impact: high, effort: large, plan: docs/opus/ENH-009-mcp-vault-resolution.md)
