...
- **[ARC-007] Cross-language vault-resolution duplication (acknowledged, gated by fixtures)** — `core/vault_path.py:340-375` (`resolve_vault`) and `visualizer/lib/vaultResolver.ts:174+` (`resolveVault`). Parity is policed by `tests/fixtures/parity/vault-resolution.json` + dual tests, but the TS resolver supports fewer channels (no `cwd/.claude/vault`, no `CLAUDE_VAULT`) — a user who sets `CLAUDE_VAULT` sees a different vault in the visualizer than in hooks. Long-term fix is ENH-009 (serve resolution through parsidion-mcp). **Depends on SEC-P001.**
...
4. **ARC-007** (→ ENH-009) — serve vault resolution through parsidion-mcp to eliminate the Python/TS duplication.
